...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return 'لینک از قبل ممنوع بود 🔒'
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'لینک ممنوع شد🔒'
  end
end
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return 'اضافه کردن ربات از قبل ممنوع بود🔒'
  else
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return 'اضافه کردن ربات از قبل آزاد بود🔓'
  else
...
local function unlock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == 'no' then
    return 'فحش دادن از قبل آزاد بود🔓'
  else
...
local function lock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'yes' then
    return 'فوروارد از قبل ممنوع بود🔒'
  else
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return 'اسپم کردن از قبل ممنوع بود🔒'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'اسپم کردن ممنوع شد🔒'
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return 'اسپم کردن از قبل آزاد بود🔓'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return 'اسپم کردن آزاد شد🔓'
  end
end
--------
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'جوین شدن وادد کردن کاربران از قبل ممنوع بود🔒'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'جوین شدن و اددکردن کاربران ممنوع شد🔒'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'جوین شدن و ادد کردن کاربران از قبل آزاد بود🔓'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return 'جوین شدن و ادد کردن کاربران آزاد شد🔓'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return 'آر تی ال از قبل ممنوع بود🔒'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'آر تی ال ممنوع شد🔒'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return 'آر تی ال از قبل آزاد بود🔓'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return 'آر تی ال آزاد شد🔓'
  end
end
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return 'پیام سرویسی تلگرام از قبل قفل بود🔒'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'پیام سرویسی تلگرام (پیام جوین شدن و ادد کردن) قفل شد🔒'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return 'پیام سرویسی تلگرام از قبل غیر قعال بود🔓'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return 'پیام سرویسی تلگرام ( پیام جوین شدن وادد کردن) آزاد شد🔓'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'فرستادن استیکر از قبل ممنوع بود🔒'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'فرستادن استیکر ممنوع شد🔒'
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return 'فرستادن استیکر از قبل آزاد بود🔓'
  else
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'yes' then
    return 'شِیر کردن شماره از قبل ممنوع بود🔒'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'شِیر کردن شماره ممنوع شد🔒'
  end
end
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return 'شِیر کردن شماره از قبل آزاد بود🔓'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'no'
    save_data(_config.moderation.data, data)
    return 'شِیر کردن شماره آزاد شد🔓'
  end
end
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return 'قفل سختگیرانه فعال شد\n-ازین پس کاربران با رعایت نکردن ممنوعات اخراج خواهند شد'
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return 'قفل سختگیرانه از قبل غیرفعال بود'
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل سختگیرانه غیرفعال شد\n-ازین پس کاربران با رعایت نکردن ممنوعات اخراج نخواند شد'
  end
end
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return 'قوانین سوپرگروه تنظیم شد و شما با فرستادن "قوانین" میتوانید آن را مشاهده کنید'
end
...
local function run(msg, matches)
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add' or matches[1] == 'فعال' and not matches[2] then
		...
		if matches[1] == 'rem' or matches[1] == 'خاموش' and is_admin1(msg) and not matches[2] then
		...
		if matches[1] == "kicked" or matches[1] == 'افراد اخراج شده' and is_momod(msg) then
		...
		if matches[1]:lower() == 'newlink' or matches[1] == 'لینک جدید' and is_momod(msg)then
		...
		if matches[1]:lower() == "setabout" or matches[1] == 'تنظیم درباره' and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1]:lower() == 'clean' or matches[1] == 'پاکسازی' then
			if not is_momod(msg) then
				return
			end
			if not is_momod(msg) then
			...
			if matches[2]:lower() == 'modlist' or matches[1] == 'مدیران' then
				if next(data[tostring(msg.to.id)]['moderators']) == nil then
			...
			if matches[2]:lower() == 'about' or matches[2] == 'درباره' then
				local receiver = get_receiver(msg)
				local about_text = ' '
				local data_cat = 'description'
				if data[tostring(msg.to.id)][data_cat] == nil then
			...
			if matches[2]:lower() == 'mutelist' or matches[2] == 'افراد سکوت' then
				chat_id = msg.to.id
			...
			if matches[2]:lower() == "bots" or matches[2] == 'ربات ها' and is_momod(msg) then
		...
		if matches[1]:lower() == 'lock' or matches[1] == 'قفل' and is_momod(msg) then
			local target = msg.to.id
			if matches[2]:lower() == 'links' or matches[2] == 'لینک' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked link posting ")
				return lock_group_links(msg, data, target)
			end
			if matches[2]:lower() == 'spam' or matches[2] == 'اسپم' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked spam ")
				return lock_group_spam(msg, data, target)
			end
			if matches[2]:lower() == 'flood' or matches[2] == 'فلود' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked flood ")
				return lock_group_flood(msg, data, target)
			end
			if matches[2]:lower() == 'bot' or matches[2] == 'ربات' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked bots ")
				return lock_group_bots(msg, data, target)
			end
			if matches[2]:lower() == 'fwd' or matches[2] == 'فوروارد' then
			...
			if matches[2]:lower() == 'arabic' or matches[2] == 'عربی' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked arabic ")
				return lock_group_arabic(msg, data, target)
			end
			if matches[2]:lower() == 'member' or matches[2] == 'اعضا' then
			...
			if matches[2]:lower() == 'sticker' or matches[2] == 'استیکر' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked sticker posting")
				return lock_group_sticker(msg, data, target)
			end
			if matches[2]:lower() == 'contacts' or matches[2] == 'شماره' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked contact posting")
				return lock_group_contacts(msg, data, target)
			end
			if matches[2]:lower() == 'strict' or matches[2] == 'سختگیرانه' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked enabled strict settings")
				return enable_strict_rules(msg, data, target)
			end
		end

		if matches[1]:lower() == 'unlock' or matches[1] == 'بازکردن' and is_momod(msg) then
			local target = msg.to.id
			if matches[2]:lower() == 'links' or matches[2] == 'لینک' then
		...
		if matches[1]:lower() == 'mute' or matches[1] == 'قفل' and is_owner(msg) then
			local chat_id = msg.to.id
			if matches[2]:lower() == 'audio' or matches[2] == 'صدا' then
			local msg_type = 'Audio'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "صدا(وویس) ممنوع شد🔒"
				else
					return "صدا(وویس)از قبل ممنوع بود🔒"
				end
			end
			if matches[2]:lower() == 'photo' or matches[2] == 'عکس' then
			local msg_type = 'Photo'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "فرستادن عکس ممنوع شد🔒"
				else
					return "فرستادن عکس از قبل ممنوع بود🔒"
				end
			end
			if matches[2]:lower() == 'video' or matches[2] == 'فیلم' then
			local msg_type = 'Video'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return "فیلم از قبل ممنوع بود🔒"
				end
			end
			if matches[2]:lower() == 'gifs' or matches[2] == 'گیف' then
			local msg_type = 'Gifs'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "گیف(تصاویرمتحرک) ممنوع شد🔒"
				else
					return "گیف(تصاویرمتحرک) از قبل ممنوع بود🔒"
				end
			end
			if matches[2]:lower() == 'documents' or matches[2] == 'فایل' then
			local msg_type = 'Documents'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "فایل ممنوع شد🔒\nفرمت های Txt,bat,exe,psd,... پاک خواهند شد"
				else
					return "فایل از قبل ممنوع بود🔒"
				end
			end
			if matches[2]:lower() == 'text' or matches[2] == 'متن' then
			local msg_type = 'Text'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "نوشته(متن و چت) ممنوع شد🔒"
				else
					return "نوشته(متن و چت) از قبل ممنوع بود🔒"
				end
			end
			if matches[2]:lower() == 'all' or matches[2] == 'همه' then
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "حالت سکوت فعال شد🔒"
				else
					return "حالت سکوت از قبل فعال بود🔒"
				end
			end
		end
		if matches[1]:lower() == 'unmute' or matches[1] == 'بازکردن' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2]:lower() == 'audio' or matches[2] == 'صدا' then
			local msg_type = 'Audio'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return "صدا(وویس) آزاد شد🔓"
				else
					return "صدا(وویس) از قبل آزاد بود🔓"
				end
			end
			if matches[2]:lower() == 'photo' or matches[2] == 'عکس' then
			local msg_type = 'Photo'
			...
			local msg_type = 'Gifs'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return "گیف(تصاویرمتحرک) آزاد شد🔓"
				else
					return "گیف(تصاویرمتحرک) از قبل آزاد بود🔓"
				end
			end
			if matches[2]:lower() == 'documents' or matches[2] == 'فایل' then
			local msg_type = 'Documents'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return "فایل آزاد شد🔓\nفرمت های Txt,bat,exe,psd,... پاک نخواهند شد"
				else
					return "فایل از قبل آزاد بود🔓"
				end
			end
			if matches[2]:lower() == 'text' or matches[2] == 'متن' then
			local msg_type = 'Text'
				if is_muted(chat_id, msg_type..': yes') then
					...
					unmute(chat_id, msg_type)
					return "نوشته(متن و چت) آزاد شد🔓"
				else
					return "نوشته(متن و چت) از قبل آزاد بود🔓"
				end
			end
			if matches[2]:lower() == 'all' or matches[2] == 'همه' then
			local msg_type = 'All'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return "حالت سکوت غیر فعال شد🔓"
				else
					return "حالت سکوت از قبل غیرفعال بود🔓"
				end
			end
		end


		if matches[1]:lower() == "muteuser" or matches[1] == 'سکوت' and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1]:lower() == "muteuser" or matches[1] == 'سکوت' and matches[2] and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
			...
			elseif matches[1]:lower() == "muteuser" or matches[1] == 'سکوت' and matches[2] and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1]:lower() == "muteslist" or matches[1] == "رسانه ها" and is_momod(msg) then
			local chat_id = msg.to.id
		...
		if matches[1]:lower() == "mutelist" or matches[1] == 'افراد سکوت' and is_momod(msg) then
			local chat_id = msg.to.id
		...
		if matches[1]:lower() == 'setgpmodel' or matches[1] == 'مدل گروه' then
	  if not is_sudo(msg) then
       return "فقط برای سودو❗️"
      end
      if matches[2]:lower() == 'realm' or matches[2] == 'ریلیم' then
      ...
      if matches[2]:lower() == 'support' or matches[2] == 'ساپورت' then
      ...
      if matches[2]:lower() == 'feedback' or matches[2] == 'پشتیبانی' then
      ...
      if matches[2]:lower() == 'vip' or matches[2] == 'اختصاصی' then
	    ...
	    if matches[2]:lower() == 'free' or matches[2] == 'رایگان' then
        ...
        return 'Group model has been changed to free'
	     ...
	     if matches[2]:lower() == 'name' or matches[2] == 'نام' then
      ...
      if matches[2]:lower() == 'normal' or matches[2] == 'متوسط' then
